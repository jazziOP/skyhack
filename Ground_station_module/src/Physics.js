...
function calculateMissionCost(totalEnergyGJ, durationDays) {
  // Initial setup and infrastructure costs (one-time, amortized)
  const initialSetupCost = 500000; // $500K for station setup, calibration, testing
  const infrastructureCost = 300000; // $300K for initial equipment, tracking systems
  const baseCost = initialSetupCost + infrastructureCost;
  
  // Energy costs
  const electricityCostPerKWh = 0.12; // USD (industrial rate)
  const energyKWh = totalEnergyGJ * 277.778; // Convert GJ to kWh
  const electricityCost = energyKWh * electricityCostPerKWh;
  
  // Laser system costs
  const laserEfficiency = 0.30; // 30% wall-plug efficiency (realistic for high-power lasers)
  const actualElectricityCost = electricityCost / laserEfficiency; // Account for inefficiency
  
  // Operating costs
  const staffingCostPerDay = 25000; // Specialized team: operators, engineers, safety personnel
  const facilityCostPerDay = 15000; // Facility operations, cooling, security
  const trackingCostPerDay = 5000; // Tracking systems, radar, telemetry
  const operatingCost = (staffingCostPerDay + facilityCostPerDay + trackingCostPerDay) * durationDays;
  
  // Maintenance and consumables
  const opticsMaintenanceCost = totalEnergyGJ * 500; // $500/GJ for optics wear, cleaning, alignment
  const coolingSystemCost = totalEnergyGJ * 200; // $200/GJ for cooling system operation
  const consumablesCost = totalEnergyGJ * 150; // $150/GJ for misc consumables
  const maintenanceCost = opticsMaintenanceCost + coolingSystemCost + consumablesCost;
  
  // Depreciation and capital costs (amortized)
  const laserSystemValue = 50000000; // $50M laser system
  const facilityValue = 30000000; // $30M facility
  const equipmentLifespan = 3650; // 10 years in days
  const depreciationCost = ((laserSystemValue + facilityValue) / equipmentLifespan) * durationDays;
  
  // Insurance and overhead
  const insuranceCostPerDay = 2000;
  const insuranceCost = insuranceCostPerDay * durationDays;
  
  const totalCost = baseCost + actualElectricityCost + operatingCost + maintenanceCost + depreciationCost + insuranceCost;
  
  // Traditional ADR methods cost comparison
  const spacecraftBasedCost = 150000000; // $150M per spacecraft mission (e.g., RemoveDEBRIS)
  const netGunCost = 80000000; // $80M for net capture system
  const harpoonCost = 100000000; // $100M for harpoon system
  const electricPropulsionTugCost = 200000000; // $200M for ion tug (e.g., ClearSpace-1)
  ...
  return {
    ...
    insuranceCost,
    totalCost,
    costPerDay: durationDays > 0 ? totalCost / durationDays : totalCost,
    energyBreakdown: {
      rawEnergy: electricityCost,
      efficiency: laserEfficiency,
      actualCost: actualElectricityCost
    },
    // Comparison with traditional methods
    comparison: {
      spacecraftBased: spacecraftBasedCost,
      netGun: netGunCost,
      harpoon: harpoonCost,
      electricTug: electricPropulsionTugCost,
      savings: spacecraftBasedCost - totalCost,
      savingsPercent: ((spacecraftBasedCost - totalCost) / spacecraftBasedCost) * 100,
      costFactor: spacecraftBasedCost / totalCost
    }
  };
}

export { calculateMissionCost };
...
function exampleMission() {
  ...
  console.log(`Cost vs Spacecraft: ${missionPlan.estimatedCost.comparison.costFactor.toFixed(0)}x cheaper`);
